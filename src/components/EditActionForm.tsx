...
const ColorContainer = styled.div`
  ...
  padding: 10px 0;
`
const ColorItem = styled.div<{ color: string; focusKey?: string }>`
  cursor: pointer;
  height: 50px;
  background-color: ${({ color }) => color};
  border-radius: 5px;
  border: ${({ color, focusKey }) =>
    color === focusKey ? '2px solid rgb(55, 53, 47)' : 'none'};
`
...
type Props = {
  onClick: () => void
  changeName: (newState: string) => void
  changeFocusKey: (newState: string) => void
  changeColor: (newState: string) => void
  changeForce?: () => void
  deleteAction?: () => void
  title: string
  buttonName: string
  ...
  color?: string
  focusKey?: string
  choiceModal?: JSX.Element
  changeIsModal?: () => void
}

const EditActionForm: React.FC<Props> = ({
  onClick,
  changeName,
  changeFocusKey,
  changeColor,
    ...
    <Overlay>
      <Container>
        ...
        <ButtonContainer>
          ...
          {isDeleteBtn ? (
            <DeleteButton onClick={changeIsModal}>
              <DeleteIcon icon={faTrashAlt as IconDefinition} />
            </DeleteButton>
          ) : null}
        </ButtonContainer>
      </Container>
